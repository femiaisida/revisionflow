...
export default function Timer() {
  ...
  useEffect(() => {
    if (timer.timerFinishedSignal !== prevSignal.current) {
      ...
      playSound();
      if (timer.musicAutoStop && selectedPlaylist) {
  ...
  function playSound() {
    ...
    try {
      ...
      osc.frequency.value = freqs[selectedSound] || 880;
      osc.type = selectedSound === 'beep' ? 'square' : 'sine';
  ...
  const bgStyle = (() => {
    ...
    if (found && found.css) return { background: found.css };
    return {};
  })();
  ...
  return (
    <div ref={containerRef} style={{ minHeight: '100%', ...bgStyle, transition: 'background 0.5s', borderRadius: isFullscreen ? 0 : undefined }}>
      {/* Top controls */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 1.5rem', background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(10px)' }}>
        <h2 style={{ margin: 0, color: 'var(--text-primary)', fontSize: '1.25rem', fontWeight: 700 }}>⏱ Timer</h2>
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button onClick={() => setShowAmbient(!showAmbient)} style={btnStyle('#6c3483')}>🎨 Ambient</button>
          <button onClick={() => setShowMusic(!showMusic)} style={btnStyle('#1a5276')}>🎵 Music</button>
          <button onClick={toggleFullscreen} style={btnStyle('#1e8449')}>{isFullscreen ? '⊠' : '⛶'} {isFullscreen ? 'Exit' : 'Full'}</button>
        </div>
      </div>
      ...
      {showAmbient && (
        <div style={{ padding: '1rem 1.5rem', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '0.75rem' }}>
            {AMBIENTS.map(a => (
              <button key={a.id} onClick={() => setAmbient(a.id)} style={{ ...btnStyle(ambient === a.id ? 'var(--accent)' : '#333'), fontSize: '0.8rem', padding: '0.3rem 0.7rem' }}>{a.label}</button>
            ))}
          </div>
          <label style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', cursor: 'pointer' }}>
            📁 Upload image: <input type="file" accept="image/*" onChange={handleBgUpload} style={{ marginLeft: '0.5rem' }} />
          </label>
        </div>
      )}
      ...
      {showMusic && (
        <div style={{ padding: '1rem 1.5rem', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '0.75rem' }}>
            <label style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '0.75rem' }}>
            {PLAYLISTS.map(p => (
              <button key={p.id} onClick={() => setSelectedPlaylist(selectedPlaylist?.id === p.id ? null : p)} style={{ ...btnStyle(selectedPlaylist?.id === p.id ? 'var(--accent)' : '#333'), fontSize: '0.8rem' }}>{p.label}</button>
            ))}
          </div>
          {selectedPlaylist && (
      ...
      <div style={{ display: 'flex', gap: 0, padding: '1rem 1.5rem 0', background: 'rgba(0,0,0,0.2)' }}>
        {['countdown', 'stopwatch', 'alarm'].map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            padding: '0.6rem 1.2rem', border: 'none', cursor: 'pointer', fontWeight: 600, textTransform: 'capitalize', fontSize: '0.9rem',
        ...
        {tab === 'countdown' && (
          <>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem', justifyContent: 'center' }}>
              {PRESETS.map(p => (
                <button key={p.s} onClick={() => timer.cdSetPreset(p.s)} style={{ ...btnStyle('#333'), fontSize: '0.8rem', padding: '0.3rem 0.75rem' }}>{p.label}</button>
              ))}
              <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                <input type="number" placeholder="Custom min" value={customMinutes} onChange={e => setCustomMinutes(e.target.value)}
                  style={{ width: 90, padding: '0.3rem 0.5rem', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--surface)', color: 'var(--text-primary)', fontSize: '0.8rem' }} />
                <button onClick={() => { if (customMinutes) timer.cdSetPreset(parseInt(customMinutes) * 60); }} style={{ ...btnStyle('var(--accent)'), fontSize: '0.8rem', padding: '0.3rem 0.7rem' }}>Set</button>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              ...
              <button onClick={() => timer.cdReset()} style={bigBtn('#555')}>↺ Reset</button>
            </div>

            {/* Sound selector */}
            <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
              <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Alert sound:</span>
              {SOUNDS.map(s => (
                <button key={s.id} onClick={() => setSelectedSound(s.id)} style={{ ...btnStyle(selectedSound === s.id ? 'var(--accent)' : '#333'), fontSize: '0.75rem', padding: '0.25rem 0.6rem' }}>{s.label}</button>
              ))}
              <button onClick={playSound} style={{ ...btnStyle('#555'), fontSize: '0.75rem', padding: '0.25rem 0.6rem' }}>Test</button>
            </div>
          </>
        )}
          ...
          <>
            ...
            {timer.swLaps.length > 0 && (
              <div style={{ width: '100%', maxWidth: 320 }}>
                <h4 style={{ color: 'var(--text-secondary)', margin: '0 0 0.5rem', fontSize: '0.85rem' }}>Laps</h4>
                <div style={{ maxHeight: 180, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
                  {timer.swLaps.map((lap, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.35rem 0.75rem', background: 'var(--surface)', borderRadius: 6 }}>
                      <span style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Lap {i + 1}</span>
                      <span style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: '0.85rem', fontVariantNumeric: 'tabular-nums' }}>{fmt(lap)}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <>
            {timer.alarmFired && (
              <div style={{ padding: '1rem 1.5rem', background: '#e74c3c22', border: '1px solid #e74c3c', borderRadius: 10, color: '#e74c3c', fontWeight: 600, fontSize: '1.1rem', textAlign: 'center' }}>
                ⏰ Alarm! <button onClick={timer.clearAlarm} style={{ marginLeft: '1rem', ...btnStyle('#e74c3c'), fontSize: '0.85rem' }}>Dismiss</button>
              </div>
            )}

            {timer.alarmTarget && !timer.alarmFired && (
              <div style={{ padding: '0.75rem 1.25rem', background: 'var(--surface)', borderRadius: 8, color: 'var(--text-secondary)', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', width: '100%', maxWidth: 340 }}>
              <div style={{ background: 'var(--surface)', borderRadius: 10, padding: '1.25rem' }}>
                <h4 style={{ margin: '0 0 0.75rem', color: 'var(--text-primary)', fontSize: '0.95rem' }}>⏰ Set clock time</h4>
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                  <input type="time" value={alarmTime} onChange={e => setAlarmTime(e.target.value)}
                    style={{ flex: 1, padding: '0.5rem', borderRadius: 6, border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--text-primary)', fontSize: '1rem' }} />
                  <button onClick={setAlarmFromTime} style={bigBtn('var(--accent)')}>Set</button>
                </div>
              </div>

              <div style={{ background: 'var(--surface)', borderRadius: 10, padding: '1.25rem' }}>
                <h4 style={{ margin: '0 0 0.75rem', color: 'var(--text-primary)', fontSize: '0.95rem' }}>⏱ Countdown alarm</h4>
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
